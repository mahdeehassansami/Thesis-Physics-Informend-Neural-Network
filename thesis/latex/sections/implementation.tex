...
The full run trains all four models for 12 split configurations and three random seeds per split. The command can take a long time on CPU, so the project also supports shorter smoke-test arguments. The final repeated-seed rows are saved in \texttt{outputs/tables/split\_seed\_results\_table.csv}, and the per-split mean results are saved in \texttt{outputs/tables/final\_results\_table.csv}.
