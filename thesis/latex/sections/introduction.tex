...
This work brings together the three aspects of the bearing prognostics problem – signal processing, machine learning, and physical interpretation – in a single local workflow. Links with vibration analysis remain with RMS, Kurtosis, Crest factor and Envelope spectral-energy features at the feature extraction step. The neural models demonstrate the ability of those features to predict NRUL for all bearing runs. The physics-informed model attempts to see if simple monotonicity and fault-frequency priors offer a benefit when running to failure data is limited.

The actual figure is likely to be higher, but is intentionally conservative. The data-only feed-forward baseline achieves the lowest mean RMSE in the 12 configuration repeated-seed evaluation, and the proposed weakly physics-informed model is not quite as consistent and close to the data-only baseline. This is still useful since it will help determine where the current physical prior is inadequate and what the reasons are for the need for additional run-level prior validation before making strong claims.
\section{Limitations}

The main limitations are:

\begin{enumerate}[label=\arabic*.]
    \item The IMS dataset has a small number of total run-to-failure records, which constraints the experiment design.
    \item The selected physics prior is weaker than a full bearing degradation law. It is a regularizer based on monotonic RUL behavior and fault-frequency energy envelope.
    \item The model uses extracted and processed features, not raw vibration waveform.
    \item The experiments are run on one local PC, so training time and numerical results may vary on other machines.
\end{enumerate}

\section{Thesis Outline}

Chapter~\ref{chapter:literature} reviews bearing failure, vibration-based condition monitoring, RUL prediction, deep learning baselines, and physics-informed learning. Chapter~\ref{chapter:methodology} describes the IMS dataset, feature extraction, preprocessing, health indicator construction, model architectures, and evaluation metrics. Chapter~\ref{chapter:implementation} describes the local implementation, experiment splits, software environment, and reproducibility setup. Chapter~\ref{chapter:results} presents the results and discusses model behavior across the 12 split configurations and repeated random seeds. Chapter~\ref{chapter:conclusion} summarizes the findings, contributions, limitations, and future work.
