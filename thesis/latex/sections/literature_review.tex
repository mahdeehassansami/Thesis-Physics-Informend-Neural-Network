...
In this chapter, we briefly survey the research background of bearings RUL prediction based on the physics-informed neural network. The discussion is organized in a similar fashion to the thesis proposal, but it has been updated to include the results of the experiments performed. It starts with the failure modes of rolling-element bearings and vibration-based health monitoring, subsequently briefly summarizes the existing predictive maintenance approaches, and then discusses recent developments in physics-informed learning for bearing condition monitoring and the general physics-informed learning theory. The chapter finishes by defining the gap in the research that is filled by this thesis.
...
Based on the literature, this thesis has three gaps that are motivating. First, a lot of the Bearing RUL studies show good results, but their preprocessing, labeling of RUL, train-test split and evaluation procedure are different. Comparisons are not possible. Second, some studies rely on random splitting of data sets which may include run-specific degradation information in both training and testing sets. Third, it is not yet matured to use physics-informed learning for bearing prognosis, and it remains unclear whether a poor physical prior will enhance prediction on held-out bearings for the entire held-out test set.

A more focused and repeatable question in this thesis is how does a weak physics-informed RUL model perform in comparison to typical neural baselines in full held-out IMS bearing runs? The study is conducted based on a local project structure, explicit feature extraction, a specific RUL target, saved tables and figures, 12 train-validation-test split configurations and repeated random seeds. It is not the intent to identify a universal best model. The aim is to try a particular physics-informed formulation and to report on the results of the completed experiment.
